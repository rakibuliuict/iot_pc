...
def pre_train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        for step, (data_a, data_b) in enumerate(zip(lab_loader_a, lab_loader_b)):
            img_a, lab_a = data_a["image"].cuda(), data_a["label"].cuda()
            img_b, lab_b = data_b["image"].cuda(), data_b["label"].cuda()
            with torch.no_grad():
